...
public class MyceliumBlockMixin {
    ...
    private static boolean canSurvive(BlockState state, WorldView world, BlockPos pos) {
        ...
        if (blockState.isOf(Blocks.SNOW) && (Integer)blockState.get(SnowBlock.LAYERS) == 1) {
            ...
            return false;
        } else if ((state.isOf(DirtyThings.DIRT_SLAB) || state.isOf(DirtyThings.MYCELIUM_SLAB)) && state.get(CustomSlabBlock.getWaterlogged())) {
            return false;
        } else {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!canSurvive(state, world, pos)) {
            ...
            if (world.getLightLevel(pos.up()) >= 9) {
                BlockState blockState = Blocks.MYCELIUM.getDefaultState();
                BlockState slabBlockState = DirtyThings.MYCELIUM_SLAB.getDefaultState();
